...
\end{align}
%
Note that the decay rate $dn/dt$ has a value of $\lambda n$, called the {\bf activity}. The SI unit is the becquerel (Bq) and is defined as one decay per second. Practically, activity tells us if a sample will have enough decays for measurement. For example, for a 1 pg sample of ${}^{230}$Th, there are roughly $10^9$ thorium atoms, from Fig.~3 of \cite{Williams1}. this corresponds to less than $10^{-3}$ Bq, which seems small, but over one week and assuming 1\% of decays are captured by measurement, Fig.~4 shows about $10^4$ decay counts.
...
\end{figure}
%
This is called the {\bf Bateman equation}, but this gives too much credit for a homework-level problem, and should be referred to as {\bf two-step decay} or {\bf two-stage decay}. We have defined the ratio $\rho(t) \equiv n'(t)/n_0$. Thiss results warrants several comments and insights.
\begin{itemize}
...
\end{align}
%
An expression for the propagation of uncertainty on (\ref{parent-age}) can be found in the Appendix of \cite{Williams1}. The same appendix also contains the Bateman equation for the grandchild population (3-stage decay).

\subsection{Chronometry value of uranium isotopes}

The chronometry for each uranium isotope has a different scope of value. That is, each isotope can tell a different story with three components: the time elapsed since purification, isotope content, and enrichment history. For forensics, the 234 and 235 isotopes have the most value and will be discussed in sufficient depth. The others will be summarized.
...
The 235 isotope is interesting because is child ${}^{231}$Th is very short lived with a half-life of 25.52 hours, or about 1 day, which is nearly instantaneous on the forensic scale. Therefore, the ${}^{231}$Pa grandchild with a half-life of $\tau_{{\rm Pa}\text{-}231} = 3.276(26)\times10^4$ yr is used for forensics instead. However, on forensic timescales, the ratio is $\rho(t) = \ln(2) \cdot t/\tau_{{\rm U}\text{-}235}$ where $\tau_{{\rm U}\text{-}235} \sim {\cal O}(10^9)$, which will amplify measurement error for $\rho$ vs.~using ${}^{234}$U chronometry, which has a lifetime of order $\tau_{{\rm U}\text{-}234} \sim {\cal O}(10^5)$. In practice, using both 234 and 235 chronometries provides a consistency check for both techniques.
...
\begin{tabular}{ccccc}
  ...
  \midrule
    ...
    ${}^{235}$U & ${}^{231}$Pa & $7.04(1)\times10^8$ yr & $3.276(26)\times10^4$ yr & $\tau_{\rm p} \gg \tau_{\rm c}$ \\
  ...
  \bottomrule
\end{tabular}
\caption{Uranium chronometry summary. Uncertainty for ${}^{230}$Th/${}^{234}$U and ${}^{231}$Th/${}^{235}$U are shown in parentheses. For ${}^{235}$U, the grandchild ${}^{231}$Pa is shown, as the child ${}^{231}$Th is too short-lived to be practical with a half-life of 25.52 hours.}
\end{table}
...
The process above measures the {\bf sample age}; \ie, the time from preparation via chemical purification to measurement via mass spectroscopy. It is the {\bf model age} that measures from the time of the first purification by some production mechanism, \eg~from UF${}_6$ gas emission, to measurement. Below, the methodologies to obtain the model age follows.
